...
<template>
  ...
  <div>
    <section class="page-hero relative isolate overflow-hidden bg-[var(--color-brand-navy)] text-white">
      ...
      <div class="site-container"><div class="max-w-4xl"><span class="inline-flex items-center gap-2 border-l-2 border-[var(--color-brand-red)] pl-3 text-sm font-bold uppercase tracking-[.12em] text-white/75"><Icon class="h-[1.1em] w-[1.1em] shrink-0" name="lucide:newspaper" aria-hidden="true" /> Logistics guide</span><h1 class="mt-5 max-w-[18ch] text-[clamp(2.4rem,5.5vw,4rem)] font-[850] leading-[1.02] tracking-[-.045em] text-white">{{ post?.title }}</h1><p class="mt-6 max-w-2xl text-lg text-white/85">{{ post?.description }}</p><p v-if="post?.date" class="mt-5 text-sm font-semibold text-white/60">Published {{ post.date }}<span v-if="post.updated"> · Updated {{ post.updated }}</span></p></div></div>
    </section>
    <section class="py-12 lg:py-20"><article class="article-prose px-1"><ContentRenderer v-if="post" :value="post" /><div class="mt-12 border-t border-[var(--color-border)] pt-8"><p class="font-extrabold text-[var(--color-text-primary)]">Need this applied to your cargo file?</p><p>Send the route, cargo type and available documents for a practical clearance or freight response.</p><NuxtLink class="mt-3 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[var(--color-brand-red)] px-5 py-3 font-extrabold text-white no-underline transition hover:bg-[var(--color-brand-red-dark)]" to="/#contact">Contact Kenya Tradex</NuxtLink></div></article></section>
  </div>
</template>
