...
export default defineEventHandler(async (event) => {
  ...
  if (!recaptchaSecret) {
    throw createError({
      statusCode: 500,
      statusMessage:
        "Server reCAPTCHA configuration is incomplete. Please contact Kenya Tradex directly at info@kenyatradex.africa.",
    });
  }
  ...
  if (!smtpHost || !smtpUser || !smtpPass) {
    throw createError({
      statusCode: 500,
      statusMessage:
        "Server email configuration is incomplete. Please contact Kenya Tradex directly at info@kenyatradex.africa.",
    });
  }
  ...
  const rows = Object.entries(fields)
    .map(
      ([key, value]) =>
        `<tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:bold;width:30%;">${escapeHtml(key.replace(/[_-]/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase()))}:</td><td style="padding:8px;border-bottom:1px solid #eee;">${escapeHtml(value)}</td></tr>`,
    )
    .join("");
