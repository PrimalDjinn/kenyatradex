...
export default defineEventHandler(async (event) => {
  ...
  await db.update(schema.newsletterDeliveries)
    .set({ status: 'suppressed', updatedAt: new Date() })
    .where(and(
      eq(schema.newsletterDeliveries.subscriberId, token.id),
      or(eq(schema.newsletterDeliveries.status, 'pending'), eq(schema.newsletterDeliveries.status, 'retry'), eq(schema.newsletterDeliveries.status, 'processing'), eq(schema.newsletterDeliveries.status, 'sending'))
    ))
