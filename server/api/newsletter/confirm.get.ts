...
export default defineEventHandler(async (event) => {
  ...
  if (!token?.nonce) throw createError({ statusCode: 400, statusMessage: 'This confirmation link is invalid or expired.' })

  const confirmed = await db.update(schema.newsletterSubscribers)
    .set({ status: 'subscribed', confirmationTokenHash: null, confirmedAt: new Date(), unsubscribedAt: null, updatedAt: new Date() })
    .where(and(
  ...
  if (!confirmed.length) throw createError({ statusCode: 400, statusMessage: 'This confirmation link has already been used or replaced.' })
