...
import {
  ...
  type NewsletterPublicationSnapshot,
...
export default defineEventHandler(async (event) => {
  ...
  const queue = await db
    ...
    .innerJoin(
      ...
      eq(schema.newsletterDeliveries.subscriberId, schema.newsletterSubscribers.id),
    )
    .where(
      and(
        or(
  ...
  if (campaignIds.length) {
    await db
      ...
      .where(
        and(inArray(schema.newsletterCampaigns.id, campaignIds), eq(schema.newsletterCampaigns.status, "pending")),
      );
  }
  ...
  for (const item of queue) {
    ...
    try {
      ...
      await db
        ...
        .where(
          and(eq(schema.newsletterDeliveries.id, item.deliveryId), eq(schema.newsletterDeliveries.status, "sending")),
        );
      sent++;
    } catch (error) {
  ...
  const activeCampaigns = await db
    .select({ id: schema.newsletterCampaigns.id })
  ...
  for (const campaignId of new Set([...campaignIds, ...activeCampaigns.map((campaign) => campaign.id)]))
...
async function createMonthlyDigest(month: string, now: Date) {
  const publications = await db
    .select()
    .from(schema.newsletterPublications)
    .where(isNull(schema.newsletterPublications.digestedAt));
  if (!publications.length) return;

  const latestPublications = [
    ...publications
      .reduce((latest, publication) => {
        ...
        return latest;
      }, new Map<string, (typeof publications)[number]>())
      .values(),
  ];
  ...
  const subject =
    snapshots.length === 1 ? firstPublication.subject : `Kenya Tradex monthly update: ${snapshots.length} new guides`;
  await db.transaction(async (tx) => {
    const [campaign] = await tx
      ...
      .onConflictDoNothing({ target: schema.newsletterCampaigns.key })
      .returning();
    if (!campaign) return;
    ...
    if (subscribers.length) {
      await tx
        .insert(schema.newsletterDeliveries)
        .values(subscribers.map((subscriber) => ({ campaignId: campaign.id, subscriberId: subscriber.id })))
        .onConflictDoNothing({
          target: [schema.newsletterDeliveries.campaignId, schema.newsletterDeliveries.subscriberId],
        });
    }
    await tx
      .update(schema.newsletterPublications)
      .set({ digestedAt: now })
      .where(
        inArray(
          schema.newsletterPublications.id,
    ...
    await tx
      ...
      .set({
        ...
        status: subscribers.length ? "pending" : "completed",
        completedAt: subscribers.length ? null : now,
      })
...
async function refreshCampaign(campaignId: number) {
  const deliveries = await db
    .select({ status: schema.newsletterDeliveries.status })
    .from(schema.newsletterDeliveries)
  ...
  const activeCount = deliveries.filter((delivery) =>
    ["pending", "retry", "processing", "sending"].includes(delivery.status),
  ).length;
  await db
    .update(schema.newsletterCampaigns)
    .set({
