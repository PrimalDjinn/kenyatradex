...
import { confirmationEmail, createConfirmationNonce, createSubscriberToken, getNewsletterFrom, getNewsletterSiteUrl, getNewsletterTransport } from '../utils/newsletter'
...
export default defineEventHandler(async (event) => {
  ...
  if (!recaptchaSecret || !recaptchaResponse) throw createError({ statusCode: 400, statusMessage: 'Please complete the reCAPTCHA verification.' })
  const verification = await $fetch<{ success?: boolean }>('https://www.google.com/recaptcha/api/siteverify', {
    method: 'POST',
    body: new URLSearchParams({ secret: recaptchaSecret, response: recaptchaResponse, remoteip: getRequestIP(event) || '' })
  })
  if (!verification.success) throw createError({ statusCode: 400, statusMessage: 'reCAPTCHA verification failed. Please try again.' })
  ...
  const [subscriber] = existing
    ? await db.update(schema.newsletterSubscribers)
        .set({ name, source, status: 'pending', frequency: 'monthly', consentAt: now, confirmationTokenHash: confirmation.hash, confirmedAt: null, unsubscribedAt: null, updatedAt: now })
        .where(eq(schema.newsletterSubscribers.id, existing.id))
        .returning()
    : await db.insert(schema.newsletterSubscribers)
        .values({ name, email, source, status: 'pending', frequency: 'monthly', consentAt: now, confirmationTokenHash: confirmation.hash, updatedAt: now })
