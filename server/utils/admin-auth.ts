...
export function verifyAdminCredentials(email: string, password: string) {
  ...
  const adminEmail = String(process.env.NUXT_ADMIN_EMAIL || config.adminEmail || '').trim().toLowerCase()
...
export function createAdminSession(email: string) {
  const config = useRuntimeConfig()
  const secret = String(process.env.NUXT_ADMIN_SESSION_SECRET || config.adminSessionSecret || process.env.NUXT_ADMIN_PASSWORD || config.adminPassword || '')
