...
export function createSubscriberToken(id: number, purpose: SubscriberToken['purpose'], nonce?: string) {
  ...
  const payload = Buffer.from(JSON.stringify({ id, nonce, purpose, exp: Date.now() + maxAge } satisfies SubscriberToken)).toString('base64url')
...
export function verifyPipelineRequest(event: H3Event, body: string) {
  ...
  if (!secret || !timestamp || !signature || !Number.isFinite(timestampMs) || Math.abs(Date.now() - timestampMs) > 1000 * 60 * 5) {
...
export function getNewsletterTransport() {
  ...
  if (!host || !user || !pass) throw createError({ statusCode: 500, statusMessage: 'Newsletter SMTP is not configured.' })
...
export function getNewsletterFrom() {
  const config = useRuntimeConfig()
  const address = String(process.env.NUXT_NEWSLETTER_FROM || config.newsletterFrom || process.env.NUXT_SMTP_USER || config.smtpUser || '')
...
export function digestText(publications: NewsletterPublicationSnapshot[], unsubscribeUrl: string) {
  const articles = publications.map(publication => `${publication.title}\n${publication.summary}\n${publication.url}`).join('\n\n')
